...
public class SearchTenantsAction extends BaseAction {
	...
	public JSONObject doApiAction(HttpServletRequest request,
		...
		try {
			...
			List<TenantAndPortalConfigVO> list = vidyoService.searchTenants(
					keyword, portalId2, pageSize2, pageNo2);
			int count = vidyoService.countTenants(keyword, portalId2);
			JSONObject result = new JSONObject();
			result.put("result", "ok");
			JSONArray ja = new JSONArray();
			if (!CommonUtil.isNullOrEmpty(list)) {
				for (TenantAndPortalConfigVO item : list) {
					ja.add(item.toJson());
				}
			}
			result.put("list", ja);
