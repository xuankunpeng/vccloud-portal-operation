...
public class StoreRoomProfileAction extends BaseAction {
	...
	public JSONObject doApiAction(HttpServletRequest request,
		...
		String roomProfileName = params.get("roomProfileName");
		...
		try {
			...
			if (CommonUtil.isNullOrEmpty(roomProfileName)) {
				vidyoService.removeRoomProfile(tenantId, roomId2);
			} else {
				vidyoService.setRoomProfile(tenantId, roomId2, roomProfileName);
			}
			return RESULT_OK;
	...
	protected boolean validateInputs(Map<String, String> params) {
		...
		if (!CommonUtil.isNullOrEmpty(roomProfileName)) {
			if (!"VideoOnly".equalsIgnoreCase(roomProfileName)
					&& !"AudioOnly".equalsIgnoreCase(roomProfileName)
					&& !"NoAudioAndVideo".equalsIgnoreCase(roomProfileName)) {
				return false;
			}
		}
		return true;
